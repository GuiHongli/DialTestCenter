...
-- 上传单个安装包
-- 上传多个安装包
2）支持下载软件包：仅ADMIN、OPERATOR、Executor有权限
-- 支持批量下载
...
2）除软件上传接口外的其他接口生成swagger 2格式的yaml文件，仅支持https,生成路径为 dialingtest-interface/src/main/resources，不使用ApiKeyAuth，通过header传递用户名的方式校验权限
3）在dialingtest-interface，pom.xml的execution节点，增加新加的yaml生成配置，执行mvn clean compile install，基于yaml自动生成接口并安装
4) 基于AR-模板.md， 生成软件实现设计文档
