...
**软件包管理功能：**
-1）支持上传软件包：仅ADMIN和OPERATOR有权限，上传方式如下：
-- 上传zip包，zip包中包含多个安装包
-- 上传单个安装包
-- 上传多个安装包
2）支持下载软件包：仅ADMIN、OPERATOR有权限
-- 支持批量下载
3）支持软件包信息的编辑，仅支持编辑描述字段
4）支持软件包的删除：仅ADMIN和OPERATOR有权限
5) 支持软件包列表的分页查询展示：所有人均有查看权限
6）软件包关键信息：软件名称（软件安装包名称，需要带后缀），描述，文件内容、文件sha256值
7）单个文件需要≤100MB，ZIP包≤1GB
8) 软件名称必须唯一,若上传同名文件，需要提示用户是否强制覆盖，若用户选择强制覆盖，实现覆盖功能
9）除预览外的所有人工操作，均需要记录到操作记录模块 

### 预处理规则管理
拨测脚本执行时，跟微调中心交互时，需要下发对应的预处理规则，故需要提前管理
**预处理规则管理功能：**
1）支持上传zip包：仅ADMIN和OPERATOR有权限，上传时支持选择业务类型
-- 上传zip包，zip包中可以嵌套目录，文件夹中放置多个预处理规则.json
2）解析json文件，提取出多条预处理规则
-- 预处理规则关键信息：规则名称，所属业务，大类，app，内容（text），是否是用户自定义
-- 规则名称：
--- 提取‘category’字段和‘apps’中的key值，拼接作为名称，该种方式提取的规则类型为非用户自定义
--- 提取‘custom_rules’字段中的key值和内层的key值，拼接作为名称，该种方式提取的规则类型为用户自定义
示例：预处理规则1-sdf_xdf.json文件，提取出来的规则名为
LIVE_douyin
LIVE_kuaishou
custome_rule_ghltest1_douyin
custome_rule_ghltest1_kuaishou
custome_rule_ghltest2_douyin
custome_rule_ghltest2_kuaishou
3）支持批量下载一个或多个规则：仅ADMIN、OPERATOR、Executor有权限
4) 支持预处理规则的分页查询展示：所有人均有查看权限
5) 规则名称联合所属业务，必须唯一,若上传同名规则，需要提示用户是否强制覆盖，若用户选择强制覆盖，实现覆盖功能
6）上传规则动作，需要记录到操作记录模块 

按照设计方案
1）操作数据库变更，保持并运行变更sql文件
2）除zip包上传接口外的其他接口生成swagger 2格式的yaml文件，仅支持https,生成路径为 dialingtest-interface/src/main/resources，使用header传递用户名的方式校验权限
3）在dialingtest-interface，pom.xml的execution节点，增加新加的yaml生成配置，执行mvn clean compile install，基于yaml自动生成接口并安装
4）基于软件实现设计文档，新增一个cotroller实现第3）步生成的Api，实现功能
- 不要新建重复的实体类，若基于yaml已生成实体类，直接使用
- dao层使用mybatis，使用在方法上写sql语句方式
6）生成软件包上传接口，使用HttpServeletRequest接收文件,功能新增到FileUploadControllor
7) 预处理规则管理前端页面，匹配后台接口调用
...
3）在dialingtest-interface，pom.xml的execution节点，增加新加的yaml生成配置，执行mvn clean compile install，基于yaml自动生成接口并安装
4) 基于AR-模板.md， 生成软件实现设计文档
5）基于软件实现设计文档，新增一个cotroller实现第3）步生成的Api，实现功能
- 不要新建重复的实体类，若基于yaml已生成实体类，直接使用
- dao层使用mybatis，使用在方法上写sql语句方式
6）生成软件包上传接口，使用HttpServeletRequest接收文件,功能新增到FileUploadControllor
7) 软件包管理前端页面，匹配后台接口调用
