...
│       │   // V3变更: onMessage改为处理ByteBuffer二进制消息
│       │
│       ├── WssMessageDispatcher.java       // (新) 核心: 消息分发器
│       │   // 职责: 解析 TLV 消息, 基于消息ID(0x01-0x36)路由到service
│       │   // V3变更: 从JSON分发改为二进制消息ID分发
│       │
│       ├── WebSocketSessionRegistry.java   // (已有) WebSocket会话注册表
│       │
│       ├── codec                           // (新V3) TLV 编解码器包
│       │   ├── TlvEncoder.java             // V3新增: TLV消息编码器
│       │   │   // 职责: 将Java对象编码为TLV二进制格式
│       │   ├── TlvDecoder.java             // V3新增: TLV消息解码器
│       │   │   // 职责: 将TLV二进制数据解码为Java对象
│       │   ├── TlvField.java               // V3新增: TLV字段基础类
│       │   │   // 职责: 表示单个TLV字段(Tag-Length-Value)
│       │   ├── MessageType.java            // V3新增: 消息类型枚举 (0x01-0x36)
│       │   │   // 定义36种消息: 注册(4)、状态(2)、UE&App(6)、脚本&任务(6)
│       │   └── FieldTag.java               // V3新增: 字段Tag枚举 (0x0001-0x0105)
│       │       // 定义所有TLV字段Tag: 基础字段(0x0001-0x0012)、
│       │       // 任务字段(0x0020-0x002C)、容器字段(0x0100-0x0105)
│       │
│       └── dto                             // (新) WSS 消息业务对象包
│           │                               // 职责: 用于业务层传递, 由TLV编解码器转换
│           │
│           ├── RegisterRequestDto.java     // (已有) 注册请求 (0x01)
│           ├── RegisterChallengeDto.java   // V3新增: 注册挑战 (0x02)
│           ├── RegisterResponseDto.java    // V3新增: 注册应答 (0x03)
│           ├── RegisterResultDto.java      // V3新增: 注册结果 (0x04)
│           ├── DeRegisterRequestDto.java   // V3新增: 去注册请求 (0x05)
│           ├── DeRegisterAckDto.java       // V3新增: 去注册应答 (0x06)
│           │
│           ├── ReportMsgDto.java           // (已有) 状态报告 (0x11)
│           ├── ReportAckDto.java           // V3新增: 状态报告应答 (0x12)
│           ├── UeItemDto.java              // V3新增: UE信息子对象
│           │
│           ├── AppListQueryDto.java        // V3新增: App清单查询 (0x21)
│           ├── AppListResponseDto.java     // V3新增: App清单应答 (0x22)
│           ├── AppItemDto.java             // V3新增: App信息子对象
│           ├── AppInstallRequestDto.java   // V3新增: App安装请求 (0x23)
│           ├── AppInstallResponseDto.java  // V3新增: App安装应答 (0x24)
│           ├── ScreanCapQueryDto.java      // V3新增: 截屏查询 (0x25)
│           ├── ScreanCapResponseDto.java   // V3新增: 截屏应答 (0x26)
│           │
│           ├── ScriptUpdateNotifyDto.java  // V3新增: 脚本更新通知 (0x31)
│           ├── ScriptUpdateAckDto.java     // V3新增: 脚本更新应答 (0x32)
│           ├── TaskStartRequestDto.java    // (已有) 任务启动请求 (0x33)
│           │   // V3变更: 新增serial-no-list、proctype字段
│           ├── TaskStartResponseDto.java   // (已有) 任务启动应答 (0x34)
│           │   // V3变更: 新增sub-result容器、files二进制字段
│           ├── SubResultItemDto.java       // V3新增: 子结果项(多UE场景)
│           ├── TaskStopRequestDto.java     // V3新增: 任务停止请求 (0x35)
│           └── TaskStopResponseDto.java    // V3新增: 任务停止应答 (0x36)
│
├── service.executormanagement              // (新) 2. 核心业务逻辑层 (设备管理)
│   │
│   ├── ExecutorMgmtService.java            // (新) [组件 3] 执行机核心服务
│   │   // V3变更: handleReportMsg接收ReportMsgDto, 发送ReportAck
│   │
│   ├── SessionBindingRegistry.java         // (已有) 会话绑定注册表
│   │   // 职责: 管理sessionId与executorName的映射关系
│   │
│   ├── ExecutorSelectionService.java       // (已有) 执行机选择服务
│   │   // 职责: 为任务选择合适的执行机
│   │
│   ├── auth                                // (新) [组件 2] 认证服务包
│   │   └── AuthSessionService.java         // 认证与会话服务
│   │       // V3变更: 实现四阶段CHAP认证 (0x01->0x02->0x03->0x04)
│   │       // 新增方法: sendRegisterChallenge, handleRegisterResponse
│   │
│   ├── task                                // (新) [组件 5] 任务接口包
│   │   ├── TaskInterfaceService.java       // 任务接口服务
│   │   │   // V3变更: 支持多UE任务(serial-no-list、proctype、sub-result)
│   │   │   // 新增方法: handleTaskStopRequest, sendTaskStopResponse
│   │   └── WssMessageSender.java           // WSS消息发送器
│   │       // V3变更: 从sendText改为sendBinary, 使用TlvEncoder编码
│   │
│   └── dto                                 // (新) 内部业务 DTO包
...
│   │                                       // [组件 4] 持久化服务
│   ├── ExecutorDao.java                    // 执行机DAO
│   │   // V3变更: updateTokenAndStatus支持8字节token
│   ├── UeDao.java                          // UE设备DAO
│   └── AgentUserDao.java                   // Agent用户DAO
│
└── model                                   // (已有) 4. 共享模型 (API + DB)
     │
     ├── Executor.java                      // (新) [组件 4] 执行机实体
     │   // V3注意: token字段需支持8字节long或256字节varchar
     ├── Ue.java                            // (新) [组件 4] UE设备实体
     └── AgentUser.java                     // (新) [组件 4] Agent用户实体
     │
...
**TLV编解码器 (5个)**
- `controller.executormanagement.websocket.codec.TlvEncoder.java`
- `controller.executormanagement.websocket.codec.TlvDecoder.java`
- `controller.executormanagement.websocket.codec.TlvField.java`
- `controller.executormanagement.websocket.codec.MessageType.java`
- `controller.executormanagement.websocket.codec.FieldTag.java`

**WebSocket DTO (23个)**
- `controller.executormanagement.websocket.dto.RegisterChallengeDto.java`
- `controller.executormanagement.websocket.dto.RegisterResponseDto.java`
- `controller.executormanagement.websocket.dto.RegisterResultDto.java`
- `controller.executormanagement.websocket.dto.DeRegisterRequestDto.java`
- `controller.executormanagement.websocket.dto.DeRegisterAckDto.java`
- `controller.executormanagement.websocket.dto.ReportAckDto.java`
- `controller.executormanagement.websocket.dto.UeItemDto.java`
- `controller.executormanagement.websocket.dto.AppListQueryDto.java`
- `controller.executormanagement.websocket.dto.AppListResponseDto.java`
- `controller.executormanagement.websocket.dto.AppItemDto.java`
- `controller.executormanagement.websocket.dto.AppInstallRequestDto.java`
- `controller.executormanagement.websocket.dto.AppInstallResponseDto.java`
- `controller.executormanagement.websocket.dto.ScreanCapQueryDto.java`
- `controller.executormanagement.websocket.dto.ScreanCapResponseDto.java`
- `controller.executormanagement.websocket.dto.ScriptUpdateNotifyDto.java`
- `controller.executormanagement.websocket.dto.ScriptUpdateAckDto.java`
- `controller.executormanagement.websocket.dto.SubResultItemDto.java`
- `controller.executormanagement.websocket.dto.TaskStopRequestDto.java`
- `controller.executormanagement.websocket.dto.TaskStopResponseDto.java`
- （已有但需补充的）
- `controller.executormanagement.websocket.dto.RegisterRequestDto.java`
- `controller.executormanagement.websocket.dto.ReportMsgDto.java`
- `controller.executormanagement.websocket.dto.TaskStartRequestDto.java`
- `controller.executormanagement.websocket.dto.TaskStartResponseDto.java`

#### 重大修改文件清单 (8个)

**WebSocket网关层 (3个)**
- `controller.executormanagement.websocket.ExecutorWebsocketEndpoint.java`
  - 修改点: `onMessage(String)` → `onMessage(ByteBuffer)`
  - 影响: 消息接收/发送机制全面改造
- `controller.executormanagement.websocket.WssMessageDispatcher.java`
  - 修改点: JSON分发 → TLV消息ID分发
  - 影响: 所有消息路由逻辑重写
- `service.executormanagement.task.WssMessageSender.java`
  - 修改点: `sendText()` → `sendBinary()`，使用TlvEncoder
  - 影响: 所有出站消息发送机制改造

**Service业务层 (5个)**
- `service.executormanagement.auth.AuthSessionService.java`
  - 修改点: 实现四阶段CHAP认证
  - 新增方法: `sendRegisterChallenge()`, `handleRegisterResponse()`
- `service.executormanagement.ExecutorMgmtService.java`
  - 修改点: 心跳处理改为接收`ReportMsgDto`，发送`ReportAck`
  - 新增方法: `sendReportAck()`
- `service.executormanagement.task.TaskInterfaceService.java`
  - 修改点: 支持多UE任务字段（serial-no-list、proctype、sub-result）
  - 新增方法: `handleTaskStopRequest()`, `sendTaskStopResponse()`
  - 文件传输: URL方式 → 直接二进制内容
- `dao.executormanagement.ExecutorDao.java`
  - 修改点: `updateTokenAndStatus()`支持8字节token
- `dao.executormanagement.UeDao.java`
  - 修改点: 方法签名调整以匹配新DTO结构

#### 删除文件清单 (1个)

- `controller.executormanagement.websocket.dto.WssMessage.java` 【删除】
  - 原因: JSON包装格式在TLV协议下不再需要

#### 轻度修改文件清单 (3个)

- `config.WebSocketJsr356Config.java` - 配置支持二进制消息
- `application.yml` - 调整WebSocket缓冲区和最大消息大小
- `dao.executormanagement.AgentUserDao.java` - 轻微方法调整

#### 变更统计汇总

| 变更类型 | 文件数量 | 说明 |
|---------|---------|------|
| 新增 | 28 | TLV编解码器5个 + DTO 23个 |
| 重大修改 | 8 | WebSocket网关3个 + Service 5个 |
| 轻度修改 | 3 | 配置和DAO调整 |
| 删除 | 1 | WssMessage.java |
| **合计** | **40** | **V3版本受影响文件总数** |

#### 实施建议

1. **第一阶段**：实现TLV编解码器（5个文件），建立基础设施
2. **第二阶段**：改造WebSocket网关（3个文件），支持二进制消息收发
3. **第三阶段**：实现所有DTO（23个文件），定义完整消息结构
4. **第四阶段**：改造Service层（5个文件），适配新协议
5. **第五阶段**：集成测试和调试工具开发
...
  2. ​**消息分发**​：作为消息分发器 (`WssMessageDispatcher`)，解析入站TLV二进制报文（按消息ID）并路由到对应的业务服务（如认证服务、执行机服务）。
  3. ​**消息出口**​：提供下行消息发送能力，供其他业务服务（如任务接口服务）调用，以向指定Agent推送指令。
* ​**输入**​:
  * Agent客户端发起的WSS连接请求。
  * Agent客户端发送的WSS TLV二进制报文（如 `Register-Request`, `Report-Msg` 等）。
  * 内部其他服务（如 [组件 5]）的下行消息发送调用（如 `sendTaskAssign`）。
* ​**输出**​:
  * 向Agent客户端推送的WSS TLV二进制报文（如 `Register-Result`, `TaskStart-Request`）。
...
activate TaskAdapter
    ...
    activate Sender
        Sender -> Wss : 3. sendBinary(sessionId,\nTaskStart-Request 0x33 TLV)
    deactivate Sender
