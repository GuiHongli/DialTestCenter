...
public class ModelTrainTaskImplTest {
    @InjectMocks
    private ModelTrainTaskImpl taskImpl;

    @Test
    public void testStart_ReturnsJobId() {
        TaskContext context = new TaskContext();
        context.setStep(TaskState.START_MODEL_TRAIN);
        String jobId = taskImpl.start(context);
        assertNotNull(jobId);
        assertTrue(jobId.startsWith("job-train-"));
    }
