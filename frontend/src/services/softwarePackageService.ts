 ...
 * 获取软件包详情
 */
export const getSoftwarePackage = async (id: number): Promise<SoftwarePackageInfo> => {
 ...
 * 上传单个软件包
 */
export const uploadSoftwarePackage = async (file: File, description?: string, overwrite: boolean = false): Promise<SoftwarePackageUploadResponse> => {
 ...
 * 上传ZIP包（批量上传）
 */
export const uploadZipPackage = async (file: File, overwrite: boolean = false, description?: string): Promise<SoftwarePackageUploadResponse> => {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('overwrite', overwrite.toString());
  if (description) {
 ...
 * 下载软件包（单个或批量）
 */
export const downloadSoftwarePackage = async (ids: number[], zipFileName?: string): Promise<void> => {
 ...
 * 更新软件包信息（仅更新描述）
 */
export const updateSoftwarePackage = async (id: number, params: SoftwarePackageUpdateParams): Promise<SoftwarePackageInfo> => {
