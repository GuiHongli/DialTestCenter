...
import { preprocessRuleService } from '../services/preprocessRuleService';
...
interface PreprocessRuleListManagementProps {
  refreshTrigger?: number;
}

const PreprocessRuleListManagement: React.FC<PreprocessRuleListManagementProps> = ({ refreshTrigger }) => {
  ...
  const loadRules = async () => {
    try {
      ...
      if (response.success) {
        ...
        message.error(response.message || translateCommon('error'));
      }
    } catch (error) {
  ...
  const loadCategoriesByBusiness = async (businessZh: string) => {
    try {
      const response = await preprocessRuleService.getCategoriesByBusiness(businessZh);
      if (response.success) {
        setFilterOptions(prev => ({ ...prev, categories: response.data }));
      }
    } catch (error) {
  ...
  const loadAppNamesByBusinessAndCategory = async (businessZh: string, category: string) => {
    try {
      const response = await preprocessRuleService.getAppNamesByBusinessAndCategory(businessZh, category);
      if (response.success) {
        setFilterOptions(prev => ({ ...prev, appNames: response.data }));
      }
    } catch (error) {
  ...
  const loadRuleNamesByBusinessAndCategoryAndApp = async (businessZh: string, category: string, appName: string) => {
    try {
      const response = await preprocessRuleService.getRuleNamesByBusinessAndCategoryAndApp(businessZh, category, appName);
      if (response.success) {
        setFilterOptions(prev => ({ ...prev, ruleNames: response.data }));
      }
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'business',
      render: (businessZh: string, record: PreprocessRule) => {
