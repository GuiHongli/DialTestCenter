...
interface PreprocessRulePackage {
  id: number;
  packageName: string;
  businessZh: string;
  businessEn: string;
...
const PreprocessRulePackageManagement: React.FC = () => {
  const { language } = useI18n();
  const { t, translateCommon } = useTranslation();
  const [packages, setPackages] = useState<PreprocessRulePackage[]>([]);
  ...
  const handleDownload = async (packageId: number, packageName: string) => {
    try {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'business',
      render: (businessZh: string, record: PreprocessRulePackage) => {
    ...
    },
    {
      ...
      key: 'actions',
      render: (record: PreprocessRulePackage) => (
        <Space>
          <Button
            type="link"
